...
PPOLO 경쟁 랭킹의 목적은 다음과 같다.

- 경쟁 중인 참가자의 현재 위치를 실시간으로 보여준다.
- 참가자는 서로 다른 시각에 기록을 시작할 수 있지만, 같은 경쟁방 안에서 공정하게 경쟁해야 한다.
- 마감 시간 시점에 최종 순위를 확정한다.
- 일시정지, 패널티, 미기록, DNF, 부정행위 상태를 구분한다.
...
초기 지원 범위:
- 목표 거리 기준 경쟁: 5km / 10km / 20km / 42km 등
- 모바일 앱 기록
- 스마트 워치 기록
- 동일 경쟁방 내 참가자 간 실시간 순위
- 마감 시간 기준 최종 순위
...
### 3.1 competition room
동일한 목표 거리, 마감 시간, 규칙을 공유하는 경쟁 단위.

### 3.2 participant session
참가자가 한 경쟁방에서 남기는 하나의 기록 세션.
...
운동을 중지한 시간.
- 기본적으로 운동시간에서 제외한다.
- 다만 누적 일시정지 시간이 허용치를 넘으면 패널티가 발생한다.
...
### 6.1 기록 허용 시간
- 경쟁 기록은 **마감 시간 이전** 에만 시작/갱신/동기화할 수 있다.
- 마감 시간 이후 서버에 도착한 기록은 반영하지 않는다.
- 마감 시간 이후에는 새로운 시작도 불가하다.

### 6.2 참가자별 시작 시각
- 같은 경쟁방의 참가자는 서로 다른 시각에 기록을 시작할 수 있다.
- 시작 시각이 달라도 같은 방 안에서 같은 경쟁으로 취급한다.
...
실시간 랭킹은 현재 시점 기준으로 참가자들의 진행 상황을 비교한다.
...
### 9.3 최종 랭킹 처리 규칙
- 마감 시각 이전에 유효하게 기록된 데이터만 사용한다.
- 마감 시각 이후 도착한 데이터는 무시한다.
- 마감 시각에 `running` 상태인 참가자는 마지막 유효 스냅샷으로 종료 처리한다.
- `finished` 상태는 최종 완료로 간주한다.
- `expired` 는 마감 전까지 유효 기록이 없는 경우다.
...
### 11.2 `dnf`
- 포기 또는 중도 종료
- 기록은 남기되 최종 순위에는 낮은 우선순위로 반영

### 11.3 `disqualified`
- GPS 조작, 거리 급증, 이중 기록, 비정상 패턴 등
- 최종 순위에서 제외하거나 최하위 처리
...
PPOLO 경쟁 랭킹은 단순 거리 비교가 아니라,
**상태 + 진행률 + 유효 시간 + 페이스 + 마감 시각** 을 함께 고려하는 알고리즘이어야 한다.

핵심은 다음이다.
- 서로 다른 시각에 시작해도 같은 경쟁으로 공정하게 비교할 것
